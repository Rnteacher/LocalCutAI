  ...
  fastify.post<{ Body: CreateProjectBody }>('/', async (request, reply) => {
    ...
    db.insert(sequences)
      .values({
        ...
        data: JSON.stringify({
          tracks: [
            { id: nanoid(12), sequenceId: seqId, name: 'V1', type: 'video', index: 0, locked: false, visible: true, muted: false, solo: false, volume: 1, pan: 0, clips: [] },
            { id: nanoid(12), sequenceId: seqId, name: 'A1', type: 'audio', index: 1, locked: false, visible: true, muted: false, solo: false, volume: 1, pan: 0, clips: [] },
          ],
        }),
        createdAt: now,
        updatedAt: now,
