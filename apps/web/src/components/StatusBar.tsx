...
export function StatusBar() {
  ...
  // Sequence info
  const seqInfo = sequences.length > 0
  ...
  return (
    <footer className="flex h-6 items-center border-t border-zinc-700 bg-zinc-800 px-4 text-xs text-zinc-500">
      ...
      <span className="flex items-center gap-3">
        ...
        {speedLabel && (
          <span className="text-green-400">{speedLabel}</span>
        )}
      ...
      <span className="ml-auto flex items-center gap-4">
        ...
        {mediaAssets.length > 0 && (
          <span>{mediaAssets.length} asset{mediaAssets.length !== 1 ? 's' : ''}</span>
        )}
        <span className="text-zinc-600">Space: play • J/K/L: shuttle • I/O: mark</span>
      </span>
    </footer>
  );
}
